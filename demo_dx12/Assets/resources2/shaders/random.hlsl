...
// init functions ----------------------------------------
[numthreads(16, 16, 1)]
void main(uint3 threads : SV_DispatchThreadID)
{
    uint2 texture_dimensions;
    m_main_texture.GetDimensions(texture_dimensions.x, texture_dimensions.y);

    if (threads.x >= texture_dimensions.x || threads.y >= texture_dimensions.y)
        return;

    uint2 cell_size = texture_dimensions / 4;

    uint checker_x = threads.x / cell_size.x;
    uint checker_y = threads.y / cell_size.y;
    float checker_value = (checker_x + checker_y) % 2;

    m_main_texture[threads.xy] = checker_value;
}
// shut functions ----------------------------------------
