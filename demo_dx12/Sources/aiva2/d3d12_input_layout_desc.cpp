...
namespace aiva2
{
    ...
    void d3d12_input_layout_desc_t::init_element_descs(shader_info_t const& shader_info)
    {
        ...
        auto const& input_struct_name = entry_for_vert_func.get_input_type(0);
        assert_t::check_bool(!std::empty(input_struct_name), "(input_struct_name) is not valid");
        auto const& input_struct_data = shader_info.get_code_block().get_struct_ref(input_struct_name);

        m_element_descs = {};

        for (auto i = size_t{}; i < input_struct_data.num_field(); i++)
        {
            auto const& input_field_data = input_struct_data.get_field_ref(i);

            auto const semantic_type = input_field_data.get_semantic_type();
            ...
            {
                auto const semantic_index = static_cast<UINT>(input_field_data.get_semantic_index());
                element_desc.SemanticIndex = semantic_index;
            }

            {
                auto const primitive_type = input_field_data.get_primitive_type();
