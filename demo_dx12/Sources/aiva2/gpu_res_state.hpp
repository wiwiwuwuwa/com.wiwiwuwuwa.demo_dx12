...
namespace aiva2
{
	...
	private:
		auto set_state_for_one_subresource_vec(D3D12_RESOURCE_STATES const state, size_t const index) -> std::vector<D3D12_RESOURCE_TRANSITION_BARRIER>;

		auto set_state_for_one_subresource_opt(D3D12_RESOURCE_STATES const state, size_t const index) -> std::optional<D3D12_RESOURCE_TRANSITION_BARRIER>;

		auto set_state_for_all_subresources_vec(D3D12_RESOURCE_STATES const state) -> std::vector<D3D12_RESOURCE_TRANSITION_BARRIER>;
