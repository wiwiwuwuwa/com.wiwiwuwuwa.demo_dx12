...
namespace aiva2
{
	...
	auto render_target_t::init_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		...
		{
			auto color_barriers = init_color_views_for_rendering();
			std::move(std::begin(color_barriers), std::end(color_barriers), std::back_inserter(barriers));
		}

		{
			auto depth_barriers = init_depth_view_for_rendering();
			std::move(std::begin(depth_barriers), std::end(depth_barriers), std::back_inserter(barriers));
		}
	...
	auto render_target_t::shut_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		...
		{
			auto color_barriers = shut_color_views_for_rendering();
			std::move(std::begin(color_barriers), std::end(color_barriers), std::back_inserter(barriers));
		}

		{
			auto depth_barriers = shut_depth_view_for_rendering();
			std::move(std::begin(depth_barriers), std::end(depth_barriers), std::back_inserter(barriers));
		}
	...
	{
		return num_color_view();
	}

	void render_target_t::rem_color_resource(size_t const index)
	{
		rem_color_view(index);
	}

	auto render_target_t::has_depth_resource() const->bool
	{
		return has_depth_view();
	}

	void render_target_t::add_color_view(std::shared_ptr<rtv_eye_t> const& color_view)
	{
	...
	auto render_target_t::get_color_view(size_t const index) const->std::shared_ptr<rtv_eye_t> const&
	{
		assert_t::check_bool(index >= 0 && index < std::size(m_color_views), "index is out of range");
		return m_color_views[index];
	}

	auto render_target_t::num_color_view() const->size_t
	{
		return std::size(m_color_views);
	}

	void render_target_t::rem_color_view(size_t const index)
	{
		assert_t::check_bool(index >= 0 && index < std::size(m_color_views), "index is out of range");
	...
	void render_target_t::set_color_view(size_t const index, std::shared_ptr<rtv_eye_t> const& color_view)
	{
		assert_t::check_bool(index >= 0 && index < std::size(m_color_views), "index is out of range");
		assert_t::check_bool(color_view, "color_view is not valid");
	...
	auto render_target_t::init_color_views_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		auto barriers = std::vector<D3D12_RESOURCE_BARRIER>{};
	...
	auto render_target_t::shut_color_views_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		auto barriers = std::vector<D3D12_RESOURCE_BARRIER>{};
	...
	auto render_target_t::get_depth_view() const->std::shared_ptr<dsv_eye_t> const&
	{
		return m_depth_view;
	}

	auto render_target_t::has_depth_view() const->bool
	{
		return m_depth_view != nullptr;
	}

	void render_target_t::set_depth_view(std::shared_ptr<dsv_eye_t> const& depth_view)
	{
		m_depth_view = depth_view;
		upd_depth_handle();
	}

	auto render_target_t::init_depth_view_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		auto barriers = std::vector<D3D12_RESOURCE_BARRIER>{};
	...
	auto render_target_t::shut_depth_view_for_rendering() const->std::vector<D3D12_RESOURCE_BARRIER>
	{
		auto barriers = std::vector<D3D12_RESOURCE_BARRIER>{};
	...
	void render_target_t::upd_color_handle()
	{
	...
	void render_target_t::upd_depth_handle()
	{
		if (!m_depth_view)
		{
			m_depth_heap = {};
