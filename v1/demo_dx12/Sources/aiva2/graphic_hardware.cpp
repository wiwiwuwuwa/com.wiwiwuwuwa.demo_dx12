...
namespace aiva2
{
	...
	void graphic_hardware_t::init_adapter()
	{
		...
		assert_t::check_hresult(get_factory().EnumAdapterByGpuPreference(0, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&basic_adapter)));
	...
	{
		assert_t::check_bool(m_info_queue, "info_queue is not valid");
		m_info_queue = {};
	}	
#endif
	...
	void graphic_hardware_t::init_swap_chain()
	{
		...
		assert_t::check_hresult(get_factory().CreateSwapChainForCoreWindow(&get_command_queue(), winrt::get_unknown(get_engine().get_window_system().get_core_window()), &desc, nullptr, basic_swap_chain.put()));
	...
	void graphic_hardware_t::init_command_list()
	{
		...
		assert_t::check_hresult(get_device().CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, &get_command_allocator(), nullptr, IID_PPV_ARGS(&basic_command_list)));
		...
		assert_t::check_hresult(specific_command_list->Close());
	...
	void graphic_hardware_t::init_fence()
	{
		...
		assert_t::check_hresult(get_device().CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&basic_fence)));
